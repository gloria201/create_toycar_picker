...
class ToyCar():
    def __init__(self,model_path = os.path.dirname(__file__)+'/best.pt',imgsz =640, conf_thres = 0.5,iou_thres = 0.5,half=False):
        self.imgsz = imgsz
        self.conf_thres = conf_thres
        self.iou_thres = iou_thres
        self.device = torch.device('cuda:0')
        self.half = half
        self.model =  model = attempt_load(model_path, map_location=self.device)#torch.load(model_path, map_location=self.device)['model'].float().fuse().eval() # to FP32
        if self.half:
    ...
    def run(self,img):
        ...
        box_conf = self.postcessing(box_conf)
        return box_conf
